...
export default function GamePage() {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:8082/city-data/fetch?city=${nextCity}`)
      .then((res) => res.json())
      .then((data: CityData) => {
  ...
  const handleChallengeFriendSubmit = async () => {
    ...
    try {
        const response = await fetch(`http://localhost:8083/user-invite?username=${uniqueUsername}&highScore=${correctAnswers}`, {
          method: "POST",
        });
        if (response.ok) {
